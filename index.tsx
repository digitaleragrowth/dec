...
const ApplicationForm = () => {
    ...
    const validate = (data: typeof formData, currentFileName: string) => {
        ...
        if (!data.phone.trim()) { errors.phone = 'Phone Number is required.'; } else if (!/^\+?[0-9\s-()]{7,}$/.test(data.phone)) { errors.phone = 'Phone Number appears to be in valid.'; }
    ...
    return (
        <>
            <section className="service-hero-section scroll-trigger fade-up">
                <div className="container">
                    <h1 className="scroll-trigger fade-up" style={{transitionDelay: '0.1s'}}>{title.split(' ').slice(0, -1).join(' ')} <strong>{title.split(' ').slice(-1)}</strong></h1>
                </div>
            </section>
            <section className="content-section">
                <div className="container">
                    <div className="service-content-grid">
                        ...
                        <div className="service-sidebar-image scroll-trigger fade-up" style={{transitionDelay: '0.3s'}}>
                            <img src={image} alt={alt} />
                        </div>
                    </div>
                    <div className="service-list-section scroll-trigger fade-up" style={{transitionDelay: '0.4s'}}>
...
const BlogPostPage = ({ data }) => {
    const { title, image, alt, category, date, content } = data;
    useEffect(() => {
        const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
        if (prefersReducedMotion) { document.querySelectorAll('.scroll-trigger').forEach(el => el.classList.add('visible')); return; }
        const observer = new IntersectionObserver(
    ...
    return (
        <>
            <section className="blog-post-hero-section scroll-trigger fade-up" style={{ backgroundImage: `url(${image})` }}>
                <div className="container">
                    <div className="blog-post-hero-content scroll-trigger fade-up" style={{transitionDelay: '0.1s'}}>
                        <div className="blog-post-meta">
                            <span>{category}</span> &bull; <span>{date}</span>
                        </div>
                        <h1>{title}</h1>
                    </div>
                </div>
            </section>
            <section className="content-section blog-post-body">
                <div className="container">
                    <div className="blog-post-content scroll-trigger fade-up" style={{transitionDelay: '0.2s'}}>
                        {content.map((p, i) => <p key={i}>{p}</p>)}
                    </div>
                    <div className="blog-post-back-link scroll-trigger fade-up" style={{transitionDelay: '0.3s'}}>
                         <AppLink href="/index.html#blog" className="read-more-btn">
...
const App = () => {
  ...
  const getPageInfo = () => {
    ...
    if (blogPageData[currentPath]) return { theme: 'light', sidebar: 'BLOG', Component: () => <BlogPostPage data={blogPageData[currentPath]} /> };
